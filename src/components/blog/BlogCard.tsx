...
export function BlogCard({ slug, image, categories, title, excerpt, author, views, date }: BlogCardProps) {
    return (
        <div className="group cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/30 bg-white dark:bg-[#252a3a] rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 border border-neutral-100 dark:border-neutral-800 flex flex-col h-full">
            ...
            <div className="p-5 flex flex-col grow">
                ...
                {categories.map((cat, index) => (
                    <Badge key={index} variant="secondary" className="bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 border-none text-[10px] font-medium px-2 py-0">{cat}</Badge>
                ))}
                </div>
                <h3 className="text-lg font-bold text-neutral-900 dark:text-white mb-2 line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{title}</h3>
